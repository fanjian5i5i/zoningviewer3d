...
export default class BuildingConstructionComplete extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            sceneLayer: null
    ...
    componentDidUpdate(){
        var that = this;
        var sqlStr = ""
        if(this.state.sceneLayer && this.props.SQLstrConComp){
          // console.log(this.props.SQLstr.toString())
          // this.props.SQLstr.forEach(function(str){
          //   sqlStr+="'"+str+"'";
          // })
          const sqlStr = that.props.SQLstrConComp.map(str => "'" + str +"'");
